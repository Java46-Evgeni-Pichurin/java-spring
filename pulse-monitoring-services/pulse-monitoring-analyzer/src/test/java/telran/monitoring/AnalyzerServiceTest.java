...
class AnalyzerServiceTest {
	...
	@BeforeEach
	void redisMocking() {
		when(analyzerRepository.findById(PATIENT_NO_REDIS_DATA)).thenReturn(Optional.empty());
		when(analyzerRepository.findById(PATIENT_NO_JUMP)).thenReturn(Optional.of(probeNoJump));
		when(analyzerRepository.findById(PATIENT_JUMP)).thenReturn(Optional.of(probeJump));
	}
